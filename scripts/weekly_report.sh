...
if [ ${APPRISE_WEEKLY_REPORT} == 1 ];then
	...
	$HOME/BirdNET-Pi/birdnet/bin/apprise -vv -t "${firstLine}" -b "${NOTIFICATION}" --attach $HOME/BirdNET-Pi/BirdDB.txt --input-format=html --config=$HOME/BirdNET-Pi/apprise.txt
fi